...
public class RenderTrack extends TileEntitySpecialRenderer<TileEntityTrackStructure>{
	...
	@Override
	public void renderTileEntityAt(TileEntityTrackStructure tileTrack, double x, double y, double z, float partialTicks, int destroyStage){
		super.renderTileEntityAt(tileTrack, x, y, z, partialTicks, destroyStage);
		if(tileTrack.curve != null){
			TileEntity trackTileEntity = tileTrack.getWorld().getTileEntity(tileTrack.getPos().add(tileTrack.curve.endPos));
			...
			if(tileTrack.connectedTrack != null){
				if(tileTrack.connectedTrack.isInvalid()){
